...
teensy_declare_library(M2Radio        M2Radio              https://github.com/newdigate/M2Radio         d4a871074e19f191a3d6cbaf19c3d3bfebb9ac8b .) # subdir chosen by the importer: import_evkb_library(M2Radio sdio iw416 [lwip]). W17/W18: dual-BSS RX tag + sendDataFrameBss + uAP netif + WPA2 TLVs. Supersedes the arduino line's 27c9264, which is an ANCESTOR of this SHA (checked, not assumed) -- so this pin carries the WiFiClass facade too -- m2_uap_lwip will NOT BUILD against anything older (verified: -DEVKB_FORCE_FETCH=ON against the previous pin fails on iw416NetifInitUap/PollDual/rxFramesByBss). BT-1 (2026-08-24) adds hci/ -- H4Parser/Hci/HciEvents/BtFwLoader/HciTransport/HciPump, the Bluetooth HCI transport and the NXP V3 UART firmware download; networking/m2_hci_probe is the first example to link it and will not CONFIGURE against anything older (import_evkb_library(M2Radio ... hci) raises FATAL_ERROR on the missing subdir). 2026-09-02: Hci::service() clock-underflow fix (commands submitted from inside the drain timed out in the same pass; host test 15) -- verified fresh-user: FORCE_FETCH build + both m2_hci_probe gates against the fetched source. 2026-09-02: HciTransport::rebaud() (end()+begin()) for the BT-3 phase-0 vendor baud switch. BT-3 phase 1 adds bt/ (L2cap, BtLink, Sdp, Avdtp; Sbc in phase 3) -- first example to link it is networking/m2_hci_probe; will not CONFIGURE against anything older (import_evkb_library(M2Radio ... bt) FATAL_ERRORs on the missing subdir).
