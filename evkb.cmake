...
teensy_declare_library(cores          teensy-cores/${EVKB_CORE_SUBDIR} https://github.com/newdigate/teensy-cores    a9b0de56feb1dd7d71ad24cf63355c6d97efb10b ${EVKB_CORE_SUBDIR}) # on cores MASTER since 2026-08-20 (the m2-serial2 branch was fast-forwarded into it); it carries imxrt1176 Serial2 on LPUART2, the M.2 Bluetooth HCI UART, and (BT-1, 2026-08-24) HardwareSerial addMemoryForRead/Write -- which networking/m2_hci_probe NEEDS: the 1 KB RX ring it installs is the only slack on a port with no flow control, and the example will not COMPILE against anything older. 2026-09-03: BT-3 phase-4 RXRTSE -- Serial2.attachRts() opt-in hardware RECEIVER flow control on LPUART2 (MODIR RXRTSE|RTSWATER, written with RE=0, re-applied in begin() so the 3 Mbaud rebaud does not drop it) + lpuart_rts::modir() host unit test; audio/bt_tone_test's M2_BT_RTS_FLOW build is the first consumer. Additive + DEFAULT-OFF (the RTS pad also drives the ENET PHY reset via R1866), so no other built ELF changes 2026-09-05 (NEW-36): ResetHandler enables the CM7 L1 INSTRUCTION cache (arm_icache_invalidate_all() + arm_icache_enable() after the .bss zero; startup_ccr_at_reset records the ROM's CCR -- read 0x00040200, IC=0) and imxrt1176.h gains the SCB cache-control surface + arm_icache_{enable,disable,invalidate_all}(); D-cache still OFF, no MPU. Changes EVERY rt1176 ELF's startup: every gate image rebuilt + freshness-checked by symbol, swept 128 gates (127 pass, 1 dispositioned: bt_tone_test[media] times out at its 120 s budget under concurrent load and passes alone -- the load-sensitivity class); silicon: timing/icache_bench_hw (flash calls 192->8.3 cyc/call = ITCM, Sbc 1106->187 us/frame, ITCM 125) + acid_box golden 0x1479CEE8 bit-identical + LOOPSTAT svc 24.5->0.46 us/iter. The variable is not mangled, so C++ sketches see it with no extern "C". 2026-09-09 NEW-41: imxrt1176 audioPllConfigure(loopDiv, num/1e6) records the running audio-PLL fraction + audioPllTrimPpm() (+-500 ppm run-time trim through a BOUNDED ai_write; the A2DP sink clock servo calls it from the SAI ISR). sai1_audio_pll_init writes the SAME AI registers (denominator 1e6, ratio unchanged -- i2s_audio_test STAGE_A byte-identical); Audio >= ff610a2 routes AudioOutputI2S through it.
...
teensy_declare_library(PXP            PXP                  https://github.com/newdigate/PXP             354eda76bf3b3c4849db19666a5eab6303c51241 .) # Bumped 2026-09-18 (PXPOp::alphaOut, NEW-55).
...
teensy_declare_library(Audio          Audio                https://github.com/newdigate/Audio           ff610a29d20fb9b8c80e9342826905d001760ad2 .) # 2026-09-09 NEW-41: control_wm8962 real headphone volume (LOUT1/ROUT1 + VU latch) + headphoneVolume(0..127) for AVRCP absolute volume; output_i2s __IMXRT1176__ PLL bring-up now goes through the core's audioPllConfigure(30, 105600) (one implementation, trimmable) -- needs cores >= a9b0de5.
teensy_declare_library(SdFat          SdFat                https://github.com/newdigate/SdFat           681bfcf83d05beb943e3d905f15d8181bf9072c7 .)
teensy_declare_library(M2Radio        M2Radio              https://github.com/newdigate/M2Radio         b90d52b8c5ca20c4e98658fec85181e584596a32 .) # subdir chosen by the importer: import_evkb_library(M2Radio sdio iw416 [lwip]). W17/W18: dual-BSS RX tag + sendDataFrameBss + uAP netif + WPA2 TLVs. Supersedes the arduino line's 27c9264, which is an ANCESTOR of this SHA (checked, not assumed) -- so this pin carries the WiFiClass facade too -- m2_uap_lwip will NOT BUILD against anything older (verified: -DEVKB_FORCE_FETCH=ON against the previous pin fails on iw416NetifInitUap/PollDual/rxFramesByBss). BT-1 (2026-08-24) adds hci/ -- H4Parser/Hci/HciEvents/BtFwLoader/HciTransport/HciPump, the Bluetooth HCI transport and the NXP V3 UART firmware download; networking/m2_hci_probe is the first example to link it and will not CONFIGURE against anything older (import_evkb_library(M2Radio ... hci) raises FATAL_ERROR on the missing subdir). 2026-09-02: Hci::service() clock-underflow fix (commands submitted from inside the drain timed out in the same pass; host test 15) -- verified fresh-user: FORCE_FETCH build + both m2_hci_probe gates against the fetched source. 2026-09-02: HciTransport::rebaud() (end()+begin()) for the BT-3 phase-0 vendor baud switch. BT-3 phase 1 adds bt/ (L2cap, BtLink, Sdp, Avdtp; Sbc in phase 3) -- first example to link it is networking/m2_hci_probe; will not CONFIGURE against anything older (import_evkb_library(M2Radio ... bt) FATAL_ERRORs on the missing subdir). 2026-09-03: bt/Sbc -- clean-room SBC ENCODER (A2DP v1.3 §12; 8 subbands, 16 blocks, joint/stereo, loudness) + host SNR tool (ffmpeg oracle, unity round-trip 63.9 dB). Additive: no firmware example LINKS Sbc yet (phase 4 AudioOutputBluetooth will), so this bump does not change any built ELF. 2026-09-03: BtLink::setLegacyPin() -- force legacy PIN (SSP off from connect) for the IW416<->ESP32 sink; used only by the m2_hci_probe BENCH build (M2_BT_LEGACY_PIN=ON), gates do not reference it. 2026-09-03: BT-3 phase 4 adds bt/Rtp + bt/MediaPacketizer + bt/A2dpSource (AudioOutputBluetooth's transport core); audio/bt_tone_test is the first consumer. 2026-09-03: BT-3 phase-4 -- MediaPacketizer m_rd two-writer race CLOSED (drain() commits via advanceRd() under a PRIMASK critical section; drops==0 is now sound not merely quiet) + framesPerPacket() accessor (bt_tone_test logs frames_per_pkt for batching verification). Additive: only bt_tone_test's behaviour is affected. 2026-09-03: L2cap::MAX_PAYLOAD made PUBLIC (value unchanged, 700) so AudioOutputBluetooth caps its media packet at the L2CAP send limit -- the REAL phase-4 stall was an oversize media packet (965 B > 700 B Tx buffer) being dropped, not flow control (found + fixed audible on silicon). 2026-09-03: L2cap::onAclTrace() -- optional raw-ACL trace hook (in/out, before demux) for the headset AVDTP DISCOVER diagnosis; off unless set, additive. 2026-09-04: BT-3/NEW-9 headset AVDTP DISCOVER fix -- L2CAP CONFIG_REQ carries an MTU option (1004) + MAX_CHANNELS 3->5; Avdtp GetAllCapabilities (0x0C) + walks the SEP list to the SBC one (the Shokz lists MPEG first) + configures Delay Reporting + accepts the peer's DelayReport; Sdp::serve()/SdpServer answer the peer's SDP query of our AudioSource; BtLink page-timeout/no-role-switch/cancel-a-silent-page (Hci::reclaimCredit) + disconnect(). Verified on SILICON: a real Shokz OpenMove reaches AVDTP STREAMING with drops=0 (clean tone), ESP32 sink no-regression; host tests l2cap 45/avdtp 55/sdp 37/btlink 23; the m2_hci_probe [avdtp] fake peer now models the Shokz, demonstrated RED five ways. 2026-09-04: MediaPacketizer::pending() -- lets AudioOutputBluetooth batch media to full packets (drain only once framesPerPacket frames are ready), cutting the ACL packet/credit rate ~5x; fixes the residual credit-starvation drops on the Shokz (silicon: 344->69 packets/s, ring high-water 7->5, drops=0 over 6+ min). 2026-09-04: BtLink suppresses Number_Of_Completed_Packets (0x13) from its event trace -- at media rate the log flooded the injected console and throttled a busy consumer's main loop (acid_box); btlink_test proves it (25 checks). No gate output changes (gates don't count hci_event lines); display/acid_box M2_BT_OUT bench is the consumer. 2026-09-04: HciTransport TX extension (5 KB, credit-bounded) -- the ACL write no longer spins on Serial2's 64-byte ring (NEW-33 fix 1; acid_box main-loop measurement: ~130 ms/s of svc was wire time). Additive; every hci/ consumer gains 5 KB of .bss. 2026-09-06: NEW-34 piece 1 -- bt/BondTable (+BondStoreEeprom.h, header-only EEPROM store at offset 4000), BtLink::setBonds/page()/the stored-key rung (Link_Key_Request_Reply, erase on 0x05/0x06, cancel-race fix), A2dpSource::setBonds (bonded-candidate walk, empty-name wildcard), NAME_MAX->NAME_LEN (POSIX macro collision via Audio.h), Hci::run reply init before BUSY; networking/m2_hci_probe[reconnect] is the first gate to need it and will not BUILD against anything older (BondTable.h missing); audio/bt_tone_test includes the headers unconditionally so BOTH its gates need this pin. 2026-09-06: NEW-34 piece 2 -- L2cap allow-list/reset/nextInbound/creditsMin; Avdtp acceptor + MediaPacketizer variable frame length; BtLink non-blocking operation engine (startInquiry/startPage/startPair/startDisconnect + tick + incoming-page/link-state/supervision); A2dpSource one-attempt-either-direction (config adoption); BtSession lifecycle policy. bt_tone_test[lifecycle], acid_box (M2_BT_OUT) and m2_hci_probe[reconnect] all use it; will not build against an older pin (BtSession.h missing). 2026-09-06: NEW-34 piece 4 -- L2cap credit-leak instrument (pktsSent/creditsReturned/starves/starveMaxMs/clampHits + tickClock, additive; no new gate). The examples log a bt_cred heartbeat line; the soak is a bench claim. 2026-09-06: NEW-34 piece 3 -- A2dpSource::setAllowAvctp() (opt-in accept of AVCTP 0x0017, additive/default-off; the bt_tone_test M2_BT_ACCEPT_AVCTP capture build uses it). 2026-09-07: NEW-34 piece 5 -- L2cap::freeSlots() (the soak slot-leak baseline; host-tested, RED-pinned; the bt_tone_test[soak] gate reads it at every loss). 2026-09-07: L2cap reassembles ACL continuation fragments on the HCI Packet_Boundary flag (a real Shokz fragments its SDP query 17+5 -- the piece-5 soak's AVDTP-stall root cause). Hci::AclFn is a SIGNATURE CHANGE: every registered ACL thunk now takes uint8_t pb after the handle and must forward it LAST to A2dpSource/L2cap::onAcl (the default makes an omission compile and silently disable reassembly); bt_tone_test/acid_box/m2_hci_probe updated together. The [media] fake peer now fragments its SDP query, so bt_tone_test[media] will not PASS against anything older. 2026-09-08: AVDTP <1.3 sinks -- Avdtp::setPeerVersion() (A2dpSource passes the SDP AudioSink version): GET_CAPABILITIES (0x02) when < 1.3, and a General Reject of GET_ALL_CAPABILITIES (0x0C) -- the 1.3 form or the legacy two-byte [tl<<4|00][00] a Bose Mini SoundLink sends -- falls back to 0x02 (silicon 2026-09-08: the Bose stalled every attempt at 0x0C, streams after). BtLink::setInquiryLap() (LIAC bench knob, bt_tone_test M2_BT_INQUIRY_LIAC). Additive; no gate output changes. 2026-09-09: NEW-41 A2DP SINK -- bt/SbcDecoder (clean-room, the encoder's mirror), Avdtp SNK personality (setLocalSep/DelayReport sending; the SOURCE now REJECTS non-16/8/loudness SET_CONFIG and advertises 0x15 -- the encoder can produce nothing else), Sdp AudioSink record (setRole(SINK)), BtLink setIdentity/wantDiscoverable/acceptUnknown, Avrcp absolute volume (GetCapabilities now lists TWO events -- the [media] peer expects the 17-byte reply, so bt_tone_test[media] will not PASS against anything older), bt/A2dpSink + bt/BtSinkSession; plus fixes found on the way: Sbc leftover-bit distribution is SUBBAND-major (every stereo/joint stream to a real sink was mis-parsed on broadband content -- encoded bytes CHANGE, the icache_bench_hw sbc_crc witness is re-recorded), Avdtp::reset() kickoff + late media-channel adoption, start() resets on a re-attempt, session state assigned before callbacks. audio/bt_sink_test is the first consumer and will not BUILD against anything older (A2dpSink.h missing). 2026-09-09 bench (iPhone): the AVRCP Target record advertises Category 2 (amplifier) in the SINK role -- iOS sends absolute volume only to a Category 2 target -- and A2dpSink OPENS AVCTP ITSELF once STREAMING (an iPhone never opens AVRCP to a sink; real speakers initiate): volume buttons now reach the WM8962. The sink gate's source peer asserts both; SOURCE-role gates unchanged. 2026-09-09 NEW-42: Avrcp::answered() + respond() reports a three-way kind (SIGNATURE change; service() is the only caller) -- bt_sink_test's heartbeat WILL print ans= from it (added in the same branch, one commit later) and from then on will not BUILD against anything older; at this pin alone no evkb source calls answered() yet. 2026-09-10 NEW-46: BtSinkSession pairing window (enterPairing/pairingOpen/canPair + PAIR_* enums) -- bt_sink_test's sketch WILL call them (next commit on the branch) and from then on will not BUILD against anything older. 2026-09-10 NEW-46 Task 1 review: PairingEnd gains PAIR_END_CANCELLED -- disconnect() cancels the window, resume() opens none (btsinksession_test Q4/Q6/Q7). Additive; no gate output changes. 2026-09-10 NEW-46 second Task 1 review: the PAIRED close moved INTO the CONNECTING->STREAMING transition, ahead of the callbacks -- an app that disconnect()s from inside the attempt callback used to get PAIR_END_CANCELLED for a link that had streamed, so the end reason tracked callback timing rather than the wire (btsinksession_test Q7 + the new P6 pin that the two timings agree). CANCELLED now means a window that never saw STREAMING. Additive; no gate output changes. 2026-09-10 NEW-46 code-quality review: enterPairing(uint32_t now, PairingReason r = PAIR_CMD) -- the reason is a LABEL and no longer reaches setPairingWindowMs(0)'s off-switch (openWindow() takes commanded/automatic as an argument of its own), and PAIR_NONE normalises to PAIR_CMD (pairingOpen() true beside pairingReason() PAIR_NONE was a contradiction the heartbeat would have printed); three ad-hoc close sites become one guarded closeWindow(). Source-compatible with the previous pin's enterPairing(now, PAIR_CMD) call shape; no gate output changes.
...
teensy_declare_library(LVGL           LVGL                 https://github.com/newdigate/LVGL            8b0799c5e9b24c13837a6b27f641488e134d895e .) # NOT Arduino-layout: use import_evkb_lvgl(), not import_evkb_library(). Bumped 2026-09-19 (NEW-55: create_db installs the PXP alphaOut sync copy for every db consumer; eDMA copy handler + host-tested band planner); earlier 2026-09-15 (create_rotated placed in flash, ITCM headroom; earlier same day: rotated present: lvgl_mipi_panel_create_rotated + lvgl_gt911_indev_create_rotated + the pure lvgl_panel_rotation.h with its host suite, acid_box landscape; earlier: 2026-08-30 flip_sync wait_us counter, NEW-23 fps diagnosis).
teensy_declare_library(SynthUI        SynthUI              https://github.com/newdigate/SynthUI         567ad9b3f7be7419a183bb1bdf821b95386094df .) # NOT Arduino-layout: use import_evkb_synthui(). Bumped 2026-09-16 (NEW-50: set_cue damages the bezel ring as four strips, led_draw clip-aware, draw dscs carry base.obj -- pixel-neutral, verified by all four goldens not moving; earlier: 2026-09-16 (synthui_led_button bezel cue width from the palette, NEW-25 final review -- pixel-neutral, verified by the goldens not moving; earlier: 2026-09-15 synthui_led_button widget, NEW-25; earlier: Bumped 2026-09-07 (synthui_slide_toggle widget, NEW-30; earlier: synthui_piano_key widget, NEW-28; earlier: synthui_level_meter widget, NEW-26; earlier: synthui_seven_segment widget, NEW-29; earlier: synthui_panel_button widget, NEW-27; earlier: synthui_lamp widget, NEW-24; earlier: 2026-08-30 (fader GC355 compositor -- one contour per path, NEW-23; earlier: 2026-08-29 (synthui_fader widget + drag math, NEW-23; earlier: 2026-08-28 (deferred compose for flip displays + annulus well -- tear-free pipeline; gpu well + single-contour track, delta both engines, 32 fps fenced / ~42 compute; earlier: wedge-delta, one-per-pixel composite, unrotated discs; earlier same day: synthui_rotary_knob added, old knob removed)))). History was REWRITTEN before the 2026-08-17 public push (reference/rebirth/ dropped), so every SHA before e132012 is unreachable.
teensy_declare_library(VGLite         VGLite               https://github.com/newdigate/VGLite          2ecb123d9541e623dfb22fa79b753584b6d76c36 .) # NOT Arduino-layout: use import_evkb_vglite(). NXP's VGLite vendored verbatim plus this tree's bare-metal port. Since the v7 (SDK 26.06 LTS) re-vendor it is MIT THROUGHOUT -- the Apache-2.0 vg_lite_flat.{c,h} pair no longer exists upstream; NOTICE records the history.
