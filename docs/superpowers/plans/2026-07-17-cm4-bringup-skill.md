...
| Zephyr (HAL, soc, drivers) | Apache-2.0 | read-for-understanding, cross-validation; avoid copying (NOTICE burden) |
| PJRC teensy cores (`cores/teensy*`) | MIT/PJRC | uncompiled reference copies ONLY — audit part 2 proves nothing under them links |
| RT1170 RM (`rm_full.txt`) | NXP doc | facts (offsets/bits) are fine; do not paste prose |
| qemu2 (GPL-2.0-or-later) | GPL | **NEVER into firmware repos.** One-way: firmware -> qemu2 is fine |

## Rules for new CM4 code

1. Register/CMSIS headers may be vendored verbatim (BSD-3 notice kept).
...
4. Every adapted block gets that provenance comment — greppable, so the
   audit allowlist stays justified.
...
- ensure the new build's depfiles are covered by the part-2 link
  manifest walk (add the build dir if needed),
