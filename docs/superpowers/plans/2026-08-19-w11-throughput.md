...
- [ ] **Step 1:** In `port/lwipopts.h` change exactly these values (current → new):

```c
#define MEM_SIZE                     (48 * 1024)   /* was 24*1024 */
#define MEMP_NUM_TCP_SEG             40            /* was 24; >= (4*TCP_SND_BUF+TCP_MSS-1)/TCP_MSS = 33 */
...
- [ ] **Step 2:** Rebuild BOTH existing lwip consumers in this tree and re-run their gates (both must PASS unchanged — they assert reachability/fallback, not window sizes):
...
- [ ] **Step 3:** Commit in `~/Development/lwip` (do NOT push; the controller pushes and bumps the pin at close-out):
...
- [ ] **Step 1:** Add zero-cost counters (plain `uint32_t`, no behaviour change), reset alongside the existing counters on fw download:
...
- [ ] **Step 2:** Build the probe + lwip examples against the local driver; run both QEMU gates (PASS):
...
- [ ] **Step 3:** Commit in `~/Development/M2Radio` (no push):
...
- [ ] **Step 1:** Write the four files; configure + build WITHOUT creds/fw (fallback image):
...
- [ ] **Step 2:** `./run_qemu.sh` → PASS; commit `transcript_qemu.txt` with the rest.
- [ ] **Step 3:** Add the GATES entry to `tools/license-audit.sh`; run the audit:

```sh
LICENSE_AUDIT_EVKB=$(pwd) ./tools/license-audit.sh   # from repo root; expect PASS
```

- [ ] **Step 4:** `python3 -m py_compile tput_peer.py` (syntax gate) and a loopback smoke of the peer's arg parsing (`./tput_peer.py --help` exits 0).
- [ ] **Step 5:** Commit (rt1176-evkb repo):
...
- [ ] Configure the example WITH creds (PSK read from the session scratchpad at configure time, lands only in the gitignored build cache + generated header) + the fw blob; build; flash with `LinkServer flash … load` (VCOM free).
- [ ] First silicon step: watch the scan/connect on serial.  **STOP CONDITION:** if `OnestreamQJN7` (2.4 GHz) is not in our scan, stop the phase and re-plan (5 GHz scan support is its own work item — do not blind-patch).
- [ ] On join + DHCP: note ip, run `python3 tput_peer.py all <ip>` on the Mac (firewall Allow prompt possible — user clicks).  Record the 4-cell table + `ps=` + `bus=` counters.
- [ ] Commit the raw numbers into `transcript_hw_evkb.txt` as the "first measurement" section (even before any fix — the baseline is evidence).

### Task 5: attribute + fix the top bottleneck (data-driven; scope = ONE fix)

- [ ] From the `bus=` counters compute per-frame and per-byte bus cost (e.g. CMD52 polls per TX frame; CMD53 bytes vs wire bytes; byte-mode share).  Write the attribution as 3-5 sentences in the transcript draft.
- [ ] Dispatch an implementer for ONLY the top item (expected candidates, in advance-guess order: CMD53 byte-mode → block mode; CMD52 poll loop tightening/batching; multi-frame service per pass).  Full review loops.  Gates re-run.
- [ ] Controller reflash + re-run `tput_peer.py all`; record the before/after delta in the transcript.  If the fix moved nothing, say so and stop (no second fix without a new plan).

### Task 6 (controller): close-out

- [ ] Final transcript section (table, delta, PS-under-load observation, WMM note — first association against a WMM AP is itself a data point for the erratum record).
- [ ] Push `~/Development/lwip` and `~/Development/M2Radio`; bump BOTH pins in `evkb.cmake`; verify with a `-DEVKB_FORCE_FETCH=ON` configure of the new example.
- [ ] Sweep from `/tmp/ev`: expect **97 passed, 0 failed, 0 SKIP** (or 96/1/0 only if the known nondeterministic dual-core gate is the red one).  Update CLAUDE.md's gate count + measured line.
- [ ] `LICENSE_AUDIT_EVKB=$(pwd) ./tools/license-audit.sh` → PASS.
- [ ] W12 handoff doc; final branch review of controller-authored commits; commit + push branch.
