...
> **As executed 2026-09-15.** Work ran on `new-25-led-button` branches in both repos; the SynthUI push was a fast-forward of SynthUI master to that branch (`b599ae1`), done by the controller. Two additions to the steps below, both from this tree's recorded traps: the SELF-BUILDING gates' dirs (`bt_sink_test/build`, `bt_tone_test/build-{soak,lifecycle,media}`, `m2_hci_probe/build-{avdtp,baud,reconnect}`) are rebuilt with the SynthUI-linking dirs, because a pin change makes them reconfigure inside their 120 s gate budget; and the vacuity total after the final review is **62** (seven led_button cases), not 58/59.
