...
Decision rule (spec §4.3 step 2): `TARGET` must cover the largest gap the phone routinely produces, in blocks (`ms / 2.9`). If `gapmax_ms` ≤ 46 (16 blocks) and `gbig` ≤ 2 over 10 min, keep **16**. If gaps of 50–80 ms are routine (`g80` in the tens), rebuild the change arm with `-DBT_SINK_TARGET=24` (`RING` stays 32 — 8 above is still a packet). If `gbig` is large with `gapmax_ms` in the hundreds, STOP and record it: that is the phone's delivery, not a buffer size, and the spec says so. Write the measured numbers and the decision into the spec §7 draft as you go.
