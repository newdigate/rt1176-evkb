...
★ **Carried from the Task 9 code-quality review (deferred, revisit here):**
- **L2CAP channel capacity vs. reverse channels.** `L2cap::MAX_CHANNELS == 3` (SDP + AVDTP-sig + AVDTP-media) and the probe leaves the SDP channel OPEN for the whole run, so if a real sink opens **two** reverse channels the pool is exhausted and `l2.connect()` for the media channel returns null → B7 fails. The Task 9 null-guards degrade this gracefully (no crash), but a real headset that opens a reverse SDP/AVDTP channel (the OneOdio does — BT-2 finding) plus its own media negotiation could starve it. If Step 2 shows this on the bench, the fix is a library change: add `L2cap::disconnect(localCid)` (queue a DISC_REQ) and tear the SDP channel down once `s_sdpDone`, **or** bump `MAX_CHANNELS` to 4 — either is a `M2Radio/bt` change with its own review, not an example edit.
- **Redundant inquiry under CONNECT (minor).** `probeInquiry()` (~10 s) runs and then `BtLink::connect()` runs its own ~12.8 s inquiry, ~25 s total. Defensible today (`probeInquiry` is the B2 gate proof and the base-path token source), but if bench latency annoys, guard `probeInquiry()` out under `M2_BT_CONNECT` — checking first that the `[avdtp]`/base gates still get their `inq:`/`inq_name:` tokens from BtLink's `btLog`.
- **`l2.begin` uses default `aclMax=1021` (cosmetic).** The card's real `ACL_Data_Packet_Length` is captured (`aclLen`) then discarded; immaterial for the signalling-only B7 path, but thread the real value through if touching this.
