...
if (bonds) {
    ...
    for i in 0..count-1:
        ...
        if (name && name[0] && b.name[0] && !strstr(b.name, name)) continue    // target-name filter; an EMPTY stored name is a wildcard
...
- (Added in the Task 4 review.) A bond with an EMPTY stored name is a
  ...
  wrong bond. Known, deliberately deferred: `m_haveLinkKey` (the flag that
  separates "stored-key success" from "the peer chose to re-pair") is still
  set by a notification for ANY address; through today's single-link
  `A2dpSource::connect()` a foreign notification cannot arrive inside our
  authentication window, so the consequence (a missed move-to-front and a
  `pairedBy()` reading `ssp` instead of `stored`) is unreachable. Piece 2, which
  may accept incoming links, must scope it to `m_bd` the same way.
