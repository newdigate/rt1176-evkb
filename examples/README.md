...
| **display** | `ssd1306_display`, `pxp_blit_test`, `lvgl_smoke_test`, `lvgl_ili9341_test`, `lvgl_rpi_panel_test`, `lvgl_rk055_panel_test` (golden human-confirmed on glass; the RPi LVGL golden pins reproducibility only), `lvgl_rk055_touch_test` (asserts LVGL's reaction — widget state/position — not pixels), `lvgl_rk055_flip_test` (golden-free; asserts flip discipline + panel-scanned-A-then-B), `lvgl_pxp_copy_bench` (PXP-vs-CPU copy correctness QEMU-gated; timings hardware-only) |
