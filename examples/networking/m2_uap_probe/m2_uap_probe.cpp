...
// ARP REPLIES ARE THE TX ORACLE.  We send an ARP request on the uAP BSS; the
// client's own IP stack answers it without any cooperation from its sketch.  A
// reply therefore proves three things at once that a TX status code cannot:
// the frame LEFT the card, it left on the RIGHT INTERFACE (nothing else is on
// this BSS), and the client PARSED it.  A "sent OK" from the driver proves only
// that the card accepted a buffer.
static uint32_t s_arpReplies = 0;
static bool     s_arpDumped  = false;
static void bssSink(void *, const uint8_t *f, uint16_t n) {
    s_bssFrames++;
    if (n >= 42 && f[12] == 0x08 && f[13] == 0x06 &&      // ethertype ARP
        f[20] == 0x00 && f[21] == 0x02) {                 // oper = 2 (reply)
        s_arpReplies++;
        if (!s_arpDumped) {
            s_arpDumped = true;
            Serial1.print("uap_arp_reply len="); Serial1.print((int)n);
            Serial1.print(" bytes=");            dumpBytes(f, n > 48 ? 48 : n);
            Serial1.println();
        }
    }
...
// Build and send one ARP request on the uAP interface, asking the client for
// its own address.  ARP is chosen over UDP deliberately: it carries no
// checksum, so a hand-built frame cannot fail for a reason unrelated to what is
// being tested, and the reply is generated by the peer's stack rather than by
// anything we control on that side.
static uint32_t s_arpSent = 0, s_arpTxFail = 0;

static void sendArpProbe() {
    // 192.168.44.1 is the gateway the client is configured with, so a request
    // from that address is traffic it already expects.
    static const uint8_t kSpa[4] = {192, 168, 44, 1};
    static const uint8_t kTpa[4] = {192, 168, 44, 50};    // the client
    uint8_t f[42];
    uint16_t o = 0;
    for (int i = 0; i < 6; i++) f[o++] = 0xFF;            // dst: broadcast
    memcpy(&f[o], s_mac, 6); o += 6;                      // src: the card
    f[o++] = 0x08; f[o++] = 0x06;                         // ethertype ARP
    f[o++] = 0x00; f[o++] = 0x01;                         // htype Ethernet
    f[o++] = 0x08; f[o++] = 0x00;                         // ptype IPv4
    f[o++] = 6;    f[o++] = 4;                            // hlen, plen
    f[o++] = 0x00; f[o++] = 0x01;                         // oper: request
    memcpy(&f[o], s_mac, 6); o += 6;                      // sender hw
    memcpy(&f[o], kSpa, 4);  o += 4;                      // sender proto
    memset(&f[o], 0, 6);     o += 6;                      // target hw: unknown
    memcpy(&f[o], kTpa, 4);  o += 4;                      // target proto
    // ★ bss_type=1.  This is the whole point: the same call with the default
    // STA addressing puts the frame on an interface that has no client.
    SdioHost::Status s = iw416.sendDataFrameBss(f, o, Iw416::BSS_TYPE_UAP, 0);
    if (s == SdioHost::OK) s_arpSent++; else s_arpTxFail++;
    (void)iw416.flushTx();   // aggregation is off, but do not depend on that
}
...
static void runBssStartSequence() {
    ...
    if (started) {
        ...
        while (millis() - holdT0 < M2_UAP_BSS_HOLD_MS) {
            ...
            // Ask once per poll, but only with a client present -- an ARP for
            // an address nobody holds proves nothing either way.
            if (count == 1) sendArpProbe();
            Serial1.print("uap_tx arp_sent=");   Serial1.print((int)s_arpSent);
            Serial1.print(" arp_txfail=");       Serial1.print((int)s_arpTxFail);
            Serial1.print(" arp_replies=");      Serial1.print((int)s_arpReplies);
            Serial1.print(" txcount=");          Serial1.println(iw416.dataTxCount());
...
void setup() {
    ...
    Serial1.printf("[APP: %s] [VER: v%u] [BUILD: %s %s]\n", "m2_uap_probe", 1, __DATE__, __TIME__);
