...
# ★ L2CAP REASSEMBLY REGRESSION (2026-09-07). The peer's SDP query of our AudioSource record is sent as TWO ACL packets
# (PB first + PB continuation, 17 + 5 bytes) -- byte-for-byte how the Shokz delivered it on silicon in the piece-5 soak,
# where a host that did not reassemble answered the truncated first packet with an SDP ErrorResponse, dropped the tail,
