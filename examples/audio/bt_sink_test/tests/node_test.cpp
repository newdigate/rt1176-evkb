...
int main() {
    {   // 1. ONE real frame, end to end.  The packet is accepted, the frame decodes into the ring, and one
        ...
        // 16384 sine here against 7744 in steady state (case 4 drains fifteen and sees the latter).  The band
    ...
    {   // 4. RING OVERRUN drops the NEW frame, never the oldest.  Dropping the oldest would mean the producer
        ...
        //    is which block comes out LAST: the 16th frame is SILENCE and the 15 before it are loud, so a
        //    drop-oldest ring would end the drain on silence.  RING is 16, so fill saturates at 15.
        Node nd; nd->begin();
        std::vector<uint8_t> loud = encodeFrame(16000), quiet = encodeFrame(0);
        for (int i = 0; i < 15; i++) feed(nd, onePacket((uint16_t)(i + 1), loud));
        CHECK(nd->fill() == 15); CHECK(nd->frames() == 15); CHECK(nd->overruns() == 0);
        feed(nd, onePacket(16, quiet));                              // the ring is full: this one is dropped
        CHECK(nd->overruns() == 1);
        CHECK(nd->fill() == 15);                                     // unchanged
        CHECK(nd->frames() == 15);                                   // it never became a frame
        for (int i = 0; i < 15; i++) nd->update();
    ...
    {   // 8. end() -- the stream was lost or closed.  It must output silence, it must NOT count that silence as
        ...
        for (int i = 0; i < 1500; i++) {                                      // hold the ring ABOVE target: the servo winds up
            while (nd->fill() < 12) feed(nd, onePacket(seq++, f));
            nd->update();
        }
    ...
    {   // 9. hold() -- the source SUSPENDed.  Same three properties as end() (silence, no underrun, trim
        ...
        uint16_t seq = 1;
        for (int i = 0; i < 800; i++) { while (nd->fill() < 12) feed(nd, onePacket(seq++, f)); nd->update(); }
