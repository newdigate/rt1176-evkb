...
gate_init
ELF="$DIR/$(gate_build_dir)/synthui_lamp_test.elf"
OUT=$(gate_capture_path "$DIR" synthui_lamp.uart)
DBG=$(gate_capture_path "$DIR" synthui_lamp.dbg)
rm -f "$OUT"
...
grep -q "lamp_scene=16 grid=4x4" "$OUT" || { echo "FAIL: scene line missing"; exit 1; }
...
# 16-lamp bank (4x4 grid: round, bar, pill across states, colors, disabled and custom hex).
# Recorded across consecutive QEMU runs (2026-09-07, NEW-24, SynthUI lamp,
...
# Demonstrated RED 2026-09-07: CRC altered to 0xDEADBEEF -> "FAIL: lamp checksum".
grep -qE "lamp_crc=0xFC0C967C\r?$" "$OUT" || { echo "FAIL: lamp checksum"; exit 1; }
...
DSEQ=$(grep -a -oE "lamp_delta_crc=0x[0-9A-F]{8}" "$OUT" | head -1 | cut -d= -f2)
DFUL=$(grep -a -oE "lamp_fresh_crc=0x[0-9A-F]{8}" "$OUT" | head -1 | cut -d= -f2)
...
grep -qE "lamp_delta_eq=PASS\r?$" "$OUT" || { echo "FAIL: delta equality"; exit 1; }
# ENGAGEMENT: the largest single invalidated area during the recorded
# 64-step segment must stay lamp-sized (measured 4900 px; bound 8000; a full
# 720x1280 screen is 921600 px) -- a change that quietly reverts set_on to
# full invalidation fails HERE and nowhere else.
DAREA=$(grep -a -oE "lamp_damage max=[0-9]+" "$OUT" | head -1 | cut -d= -f2)
[ -n "$DAREA" ] && [ "$DAREA" -gt 0 ] || { echo "FAIL: delta damage guard missing or zero"; exit 1; }
[ "$DAREA" -le 8000 ] || { echo "FAIL: delta damage not engaged (max=$DAREA)"; exit 1; }
# vsync-fence health (db pipeline): a timeout means the tear-free property
# silently degraded with every golden still green.
grep -qE "lamp_vsync flips=[0-9]+ isrs=[0-9]+ timeouts=0\r?$" "$OUT" || { echo "FAIL: vsync fence unhealthy or missing"; exit 1; }
grep -q "crc_done" "$OUT" || { echo "FAIL: no completion token"; exit 1; }
grep -q "PASS: SynthUI lamp render verified" "$OUT" || { echo "FAIL: render verification token missing"; exit 1; }
echo "PASS: SynthUI lamp render verified"
